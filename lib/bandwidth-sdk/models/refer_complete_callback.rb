...
module Bandwidth
  # This event is sent to the referCompleteUrl of the <Refer> verb when the SIP REFER request has been resolved — either the remote endpoint has accepted and redirected the call (success) or the REFER was rejected or timed out (failure). On success the original call is terminated.
  class ReferCompleteCallback < ApiModelBase
    # The event type for this webhook. For this callback, the value is `referComplete`.
    ...
    # The SIP response code received in a NOTIFY from the remote endpoint after it attempted to reach the transfer target. Present only if a NOTIFY was received before the 30-second timeout.
