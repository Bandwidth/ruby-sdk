...
describe Bandwidth::BridgeTargetCompleteCallback do
  ...
  let(:bridge_target_complete_callback_values) { Bandwidth::BridgeTargetCompleteCallback.new({
    event_type: 'bridgeTargetComplete',
  ...
  describe '#acceptable_attributes' do
    it 'expects acceptable JSON attributes to be those in the attribute map' do
      expect(Bandwidth::BridgeTargetCompleteCallback.acceptable_attributes).to eq(Bandwidth::BridgeTargetCompleteCallback.attribute_map.values)
    end
  end

  describe '#openapi_nullable' do
    it 'expects nullable attributes to be the set of nullable fields' do
      expect(Bandwidth::BridgeTargetCompleteCallback.openapi_nullable).to eq(Set.new([
        :'enqueued_time',
        :'answer_time',
        :'tag'
      ]))
  ...
  describe '#build_from_hash' do
    it 'validates instance of BridgeTargetCompleteCallback created by the build_from_hash method' do
      ...
      expect(bridge_target_complete_callback_from_hash.event_type).to eq('bridgeTargetComplete')
      expect(bridge_target_complete_callback_from_hash.event_time).to eq(Time.parse('2022-06-16T13:15:07.160Z'))
      expect(bridge_target_complete_callback_from_hash.account_id).to eq('9900000')
      expect(bridge_target_complete_callback_from_hash.application_id).to eq('04e88489-df02-4e34-a0ee-27a91849555f')
      expect(bridge_target_complete_callback_from_hash.from).to eq('+19195554321')
      expect(bridge_target_complete_callback_from_hash.to).to eq('+19195551234')
      expect(bridge_target_complete_callback_from_hash.direction).to eq(Bandwidth::CallDirectionEnum::OUTBOUND)
      expect(bridge_target_complete_callback_from_hash.call_id).to eq('c-15ac29a2-1331029c-2cb0-4a07-b215-b22865662d85')
      expect(bridge_target_complete_callback_from_hash.call_url).to eq('https://voice.bandwidth.com/api/v2/accounts/9900000/calls/c-15ac29a2-1331029c-2cb0-4a07-b215-b22865662d85')
      expect(bridge_target_complete_callback_from_hash.enqueued_time).to eq(Time.parse('2022-06-16T13:15:07.160Z'))
      expect(bridge_target_complete_callback_from_hash.start_time).to eq(Time.parse('2022-06-16T13:15:07.160Z'))
      expect(bridge_target_complete_callback_from_hash.answer_time).to eq(Time.parse('2022-06-16T13:15:18.126Z'))
      expect(bridge_target_complete_callback_from_hash.tag).to eq('custom tag')
    end
  end
