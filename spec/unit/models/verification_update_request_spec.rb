# Unit tests for Bandwidth::VerificationUpdateRequest
describe Bandwidth::VerificationUpdateRequest do
  let(:verification_update_request_default) { Bandwidth::VerificationUpdateRequest.new({
  ...
  let(:verification_update_request_values) { Bandwidth::VerificationUpdateRequest.new({
    business_address: { name: 'Bandwidth' },
    business_contact: { firstName: 'Jane' },
    message_volume: 1000,
  ...
  describe '#initialize' do
    it 'causes an ArgumentError by passing an Array to the initialize method' do
      expect {
        Bandwidth::VerificationUpdateRequest.new([])
      }.to raise_error(ArgumentError)
    end

    it 'causes an ArgumentError by passing an invalid attribute to the initialize method' do
      expect {
        Bandwidth::VerificationUpdateRequest.new({ invalid: true })
      }.to raise_error(ArgumentError)
    end
  end
  ...
  describe '#acceptable_attributes' do
    it 'expects acceptable JSON attributes to be those in the attribute map' do
      expect(Bandwidth::VerificationUpdateRequest.acceptable_attributes).to eq(Bandwidth::VerificationUpdateRequest.attribute_map.values)
    end
  end

  describe '#openapi_nullable' do
    it 'expects nullable attributes to be the set of nullable fields' do
      expect(Bandwidth::VerificationUpdateRequest.openapi_nullable).to eq(Set.new([
  ...
  describe '#build_from_hash' do
    it 'validates instance of VerificationUpdateRequest created by the build_from_hash method' do
      verification_update_request_from_hash = Bandwidth::VerificationUpdateRequest.build_from_hash({
      ...
      expect(verification_update_request_from_hash).to be_instance_of(Bandwidth::VerificationUpdateRequest)
      expect(verification_update_request_from_hash.business_address).to be_instance_of(Bandwidth::Address)
      expect(verification_update_request_from_hash.business_contact).to be_instance_of(Bandwidth::Contact)
      expect(verification_update_request_from_hash.message_volume).to eq(1000)
      expect(verification_update_request_from_hash.use_case).to eq('Customer notifications')
      expect(verification_update_request_from_hash.use_case_summary).to eq('Send customers updates about their orders')
      expect(verification_update_request_from_hash.production_message_content).to eq('Your order has shipped')
      expect(verification_update_request_from_hash.opt_in_workflow).to be_instance_of(Bandwidth::OptInWorkflow)
      expect(verification_update_request_from_hash.additional_information).to eq('extra info')
      expect(verification_update_request_from_hash.isv_reseller).to eq('My ISV')
      expect(verification_update_request_from_hash.privacy_policy_url).to eq('https://example.com/privacy')
      expect(verification_update_request_from_hash.terms_and_conditions_url).to eq('https://example.com/terms')
      expect(verification_update_request_from_hash.business_dba).to eq('Bandwidth Inc')
      expect(verification_update_request_from_hash.business_registration_number).to eq('12-3456789')
      expect(verification_update_request_from_hash.business_registration_type).to eq(Bandwidth::BusinessRegistrationTypeEnum::EIN)
      expect(verification_update_request_from_hash.business_entity_type).to eq(Bandwidth::BusinessEntityTypeEnum::PRIVATE_PROFIT)
      expect(verification_update_request_from_hash.business_registration_issuing_country).to eq('USA')
      expect(verification_update_request_from_hash.help_message_response).to eq('For help reply with HELP')
      expect(verification_update_request_from_hash.age_gated_content).to eq(false)
      expect(verification_update_request_from_hash.cv_token).to eq('cv_token_value')
    end
  end

  describe '#to_s' do
    it 'returns a string representation of the object' do
      expect(verification_update_request_values.to_s).to eq('{:businessAddress=>{:name=>"Bandwidth"}, :businessContact=>{:firstName=>"Jane"}, :messageVolume=>1000, :useCase=>"Customer notifications", :useCaseSummary=>"Send customers updates about their orders", :productionMessageContent=>"Your order has shipped", :optInWorkflow=>{:description=>"opt-in via website", :imageUrls=>["https://example.com/optin.png"]}, :additionalInformation=>"extra info", :isvReseller=>"My ISV", :privacyPolicyUrl=>"https://example.com/privacy", :termsAndConditionsUrl=>"https://example.com/terms", :businessDba=>"Bandwidth Inc", :businessRegistrationNumber=>"12-3456789", :businessRegistrationType=>"EIN", :businessEntityType=>"PRIVATE_PROFIT", :businessRegistrationIssuingCountry=>"USA", :helpMessageResponse=>"For help reply with HELP", :ageGatedContent=>false, :cvToken=>"cv_token_value"}')
    end
  end

  describe '#eq? #==' do
    it 'returns true/false when comparing objects' do
      verification_update_request_equal = Bandwidth::VerificationUpdateRequest.new({
        business_address: { name: 'Bandwidth' },
        business_contact: { firstName: 'Jane' },
        message_volume: 100,
        use_case: 'baseline use case',
      ...
      expect(verification_update_request_default.eql?(verification_update_request_equal)).to be true
      expect(verification_update_request_default.eql?(verification_update_request_values)).to be false
    end
  end

  describe '#to_body #to_hash' do
    it 'returns a hash representation of the object' do
      expect(verification_update_request_values.to_body).to eq({
  ...
  describe 'custom attribute writers' do
    ...
    it '#use_case_summary=' do
      expect {
        verification_update_request_values.use_case_summary = nil
      }.to raise_error(ArgumentError, 'use_case_summary cannot be nil')

      expect {
        verification_update_request_values.use_case_summary = 'a' * 501
    ...
    it '#production_message_content=' do
      expect {
        verification_update_request_values.production_message_content = nil
      }.to raise_error(ArgumentError, 'production_message_content cannot be nil')

      expect {
        verification_update_request_values.production_message_content = 'a' * 501
    ...
    it '#additional_information=' do
      expect {
        verification_update_request_values.additional_information = 'a' * 501
    ...
    it '#isv_reseller=' do
      expect {
        verification_update_request_values.isv_reseller = 'a' * 501
    ...
    it '#privacy_policy_url=' do
      expect {
        verification_update_request_values.privacy_policy_url = nil
      }.to raise_error(ArgumentError, 'privacy_policy_url cannot be nil')

      expect {
        verification_update_request_values.privacy_policy_url = 'a' * 501
    ...
    it '#terms_and_conditions_url=' do
      expect {
        verification_update_request_values.terms_and_conditions_url = nil
      }.to raise_error(ArgumentError, 'terms_and_conditions_url cannot be nil')

      expect {
        verification_update_request_values.terms_and_conditions_url = 'a' * 501
    ...
    it '#business_dba=' do
      expect {
        verification_update_request_values.business_dba = nil
      }.to raise_error(ArgumentError, 'business_dba cannot be nil')

      expect {
        verification_update_request_values.business_dba = 'a' * 501
    ...
    it '#business_registration_number=' do
      expect {
        verification_update_request_values.business_registration_number = 'a' * 501
    ...
    it '#help_message_response=' do
      expect {
        verification_update_request_values.help_message_response = 'a' * 501
    ...
    it '#cv_token=' do
      expect {
        verification_update_request_values.cv_token = 'a' * 501
