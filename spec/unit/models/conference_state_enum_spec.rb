...
describe Bandwidth::ConferenceStateEnum do
  describe 'constants' do
    it 'defines ACTIVE' do
      expect(Bandwidth::ConferenceStateEnum::ACTIVE).to eq('active')
    end

    it 'defines COMPLETED' do
      expect(Bandwidth::ConferenceStateEnum::COMPLETED).to eq('completed')
    end
  end

  describe '.all_vars' do
    it 'returns every valid enum value' do
      expect(Bandwidth::ConferenceStateEnum.all_vars).to eq([
        'active',
        'completed'
      ])
    end
  ...
  describe '.build_from_hash' do
    it 'returns the value when it matches a valid enum value' do
      expect(Bandwidth::ConferenceStateEnum.build_from_hash('active')).to eq('active')
      expect(Bandwidth::ConferenceStateEnum.build_from_hash('completed')).to eq('completed')
    end
